...
function Accordion({ className, expanded, defaultExpanded, onChange, children, ...props }) {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        open ? "border-primary/30 shadow-[0_1px_0_0_rgba(0,0,0,0.02),0_8px_24px_-8px_rgba(0,0,0,0.08)]" : "border-border/60 shadow-sm",
        "hover:border-border",
        className
      )}
      {...props}
      ...
      >
        ...
        <span
          className={cn(
            ...
            open ? "bg-primary/10" : "bg-muted group-hover:bg-muted-foreground/10"
          )}
        >
...
function AccordionSummary({ children, expandIcon, icon, description, ...props }) {
  return null
}
